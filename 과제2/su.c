...
void *check_rows(void *arg)
{
    // 여기를 완성하세요
    int i,j;
    int visit[9];   //방문을 한 경우에 체크하는 용도이다.

    for(i=0;i<9;++i) {
        for(j=0;j<9;++j) {
            visit[j]=0;   //0으로 초기화를 한다.
        }
        for(j=0;j<9;++j) {
            visit[sudoku[i][j]-1] = 1; //스도쿠가 값이 있는데 1~9까지 확인을 하는 용도
        }
        valid[0][i]=1;
        for(j=0;j<9;++j) {   //검사
            if(visit[j]==0) {  // 만약 스도쿠에서 1~9까지 모든 수가 존재하지 읺을 때이다.
                valid[0][i] = 0;
                break;
            }
        }
    }
    pthread_exit(NULL);   //NULL을 호출 시키고 쓰레드를 종료시키기 위함이다. 
}
...
void *check_columns(void *arg)
{
    ...
    for(j=0;j<9;++j) {
        ...
        valid[1][j]=1;
        for(i=0;i<9;++i) {   //검사
            if(visit[i]==0) {  // 만약 스도쿠에서 1~9까지 모든 수가 존재하지 읺을 때이다.
                valid[1][j] = 0;
                break;
            }
        }
...
void *check_subgrid(void *arg)
{
    ...
    k = *(int *)arg; //arg 주소의 형태가 void형태인데 이를 정수형을 바꾸어준다.
    for(i=0;i<9;++i) {
        visit[i]=0;
    }
    row = (k/3)*3;    //한칸에 9개의 숫자가있는지 확인하는데 행을 나타냄
    col = (k%3)*3;    //한칸에 9개의 숫자가있는지 확인하는데 열을 나타냄
    //3칸씩 차이가 나서 row와 col을 이렇게 두었다.
    for(i=row;i<row+3;++i) {      //9칸을 확인 해줄 것이다.
        for(j=col;j<col+3;++j) {
            visit[sudoku[i][j]-1]=1; //숫자가 1~9까지 다 있는지 확인해야한다.
        }
    }
    valid[2][k]=1; //k번째 애가 실행 되어서 1로 바꾸어준다.
    for(i=0;i<9;++i) {
        if(visit[i] == 0) {
            valid[2][k] = 0;
            break;      //visit[i]==0이여서 해당 숫자가 없어서 스도쿠를 만족하지 않음.
        }
    }
    pthread_exit(NULL);//NULL을 호출 시키고 쓰레드를 종료시키기 위함이다. 
}
...
void check_sudoku(void)
{
     ...
     */
    // 여기를 완성하세요
    if(pthread_create(&rowtid,NULL,check_rows,NULL) != 0) { //쓰레드를 생성할 때 사용하는데 0이아니면 에러여서 밑에 코드를 실행했다.
     ...
     */
    // 여기를 완성하세요
    if(pthread_create(&coltid,NULL,check_columns,NULL) != 0) { //쓰레드를 생성할 때 사용하는데 0이아니면 에러여서 밑에 코드를 실행했다.
        fprintf(stderr, "pthread_creat error!!\n");
        exit(-1);
    }
    /*
    ...
    for(i=0;i<9;++i) {
        gid[i]=i;
        if(pthread_create(subgridtid+i,NULL,check_subgrid,gid+i) != 0) { //쓰레드 생성을 9개 할건데 이애는 배열의 형태이므로 저렇게 선언한다.
            fprintf(stderr,"pthread_creat error!!\n");
            exit(-1);
        }
    }
    ...
    // 여기를 완성하세요
    for(i=0;i<9;++i) {
        pthread_join(subgridtid[i],NULL);  //subgrid가 종료될 때까지 기다린다.
    }
    pthread_join(rowtid,NULL); //row가 종료될 때 까지 기다린다.
    pthread_join(coltid,NULL); //col가 종료될 때 까지 기다린다.
    /*
     * 각 행에 대한 검증 결과를 출력한다.
     */
    printf("ROWS: ");
